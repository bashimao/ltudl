...
abstract class LinearFilterDecoder_JVM
  extends LinearFilterDecoder {
  ...
  final protected val w_t = filter.valuesMatrix

  final protected val w = w_t.t
  ...
  : RealArrayTensor = {
    ...
    val out = doPredict(inp)
    out
  }

  protected def doPredict(input: RealArrayTensor): RealArrayTensor
  ...
  : RealArrayTensor = {
    ...
    val newErr = doDeriveInputError(oldErr)
  ...
  protected def doDeriveInputError(error: RealArrayTensor): RealArrayTensor
