...
final class TestModule_InvertedDropout
  extends FlatSpec
    with Matchers {
  ...
  val initializer = UniformDistributionBuilder()

  val y = RealArrayTensor.ones(Size1(1, 2), 3)

  val e = RealArrayTensor.ones(Size1(1, 2), 3)

  "InvertedDropout" should "not have any influence when Test mode is used" in {
    ...
    // Check backward path
    val res0 = ProbeEntireBufferBuilder().build()(0L, seq0, x, e)
    val res1 = ProbeEntireBufferBuilder().build()(0L, seq1, x, e)
    res0.rating should be < tolerance1
    res1.rating should be < tolerance1
  }
