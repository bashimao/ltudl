...
import edu.latrobe.blaze.modules.{ELU, ELUBuilder}

abstract class ELU_JVM
  extends ELU
    with MapLayer_JVM[ELUBuilder] {

  val alpha: Real = builder.alpha


  // ---------------------------------------------------------------------------
  //    Forward propagation related.
  // ---------------------------------------------------------------------------
  final protected def doPredict(inPlaceAllowed: Boolean,
                                input:          Tensor)
  : RealArrayTensor = {
    val out = {
      if (inPlaceAllowed) {
