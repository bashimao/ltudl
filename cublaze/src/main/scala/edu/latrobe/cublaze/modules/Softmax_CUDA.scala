...
abstract class Softmax_CUDA
  extends Softmax
    with MapLayer_CUDA[SoftmaxBuilder] {
  ...
  protected def doPredict(inp: CUDARealTensor, out: CUDARealTensor): Unit
  ...
  protected def doDeriveInputError(out: CUDARealTensor, err: CUDARealTensor)
  : Unit
