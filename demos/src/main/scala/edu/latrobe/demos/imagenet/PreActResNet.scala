...
/**
  * Improved ResNet with preactivations:
...
object PreActResNet {
  ...
  : SequenceBuilder = {
    ...
    if (pType == "Both") {
      block += BatchNormalizationBuilder()
      block += ReLUBuilder()
    }
    else if (pType != "None") {
      filter += BatchNormalizationBuilder()
      filter += ReLUBuilder()
    }

    filter += ConvolutionFilterBuilder(Kernel2((3, 3), stride, (1, 1)), noMaps)
    filter += BatchNormalizationBuilder()
    filter += ReLUBuilder()
    filter += ConvolutionFilterBuilder(Kernel2((3, 3), (1, 1), (1, 1)), noMaps)

    val shortcut = createShortcut(stride, noMaps)

    block += BranchBuilder(filter, shortcut)
    block += MergeBuilder()

    noMapsPrev = noMaps
    block.setHandle("Basic Block")
  }
  ...
  : SequenceBuilder = {
    ...
    filter += ConvolutionFilterBuilder(Kernel2((1, 1), (1, 1), (0, 0)), noMaps)
    filter += BatchNormalizationBuilder()
    filter += ReLUBuilder()
    filter += ConvolutionFilterBuilder(Kernel2((3, 3), stride, (1, 1)), noMaps)
    filter += BatchNormalizationBuilder()
    filter += ReLUBuilder()
    filter += ConvolutionFilterBuilder(Kernel2((1, 1), (1, 1), (0, 0)), noMaps4)
    ...
    noMapsPrev = noMaps4
    block.setHandle("Bottleneck Block")
  }

  final def createBlockGroup(noBlocks:      Int,
                             stride:        (Int, Int),
                             noMaps:        Int,
                             createBlockFn: ((Int, Int), Int, String) => ModuleBuilder,
                             isFirst:       Boolean)
  : ModuleBuilder = {
    ...
    // Translation block.
    group += createBlockFn(stride, noMaps, if (isFirst) "None" else "Both")

    // Mapping blocks.
    for (i <- 1 until noBlocks) {
      group += createBlockFn((1, 1), noMaps, null)
    }

    group
  }

  final def createBlockGroup(values:  (Int, (Int, Int), Int, ((Int, Int), Int, String) => ModuleBuilder),
                             isFirst: Boolean)
  : ModuleBuilder = values match {
    case (noBlocks, stride, noMaps, createBlockFn) =>
      createBlockGroup(noBlocks, stride, noMaps, createBlockFn, isFirst)
  }

  final def createForImageNet(depth: Int, noClasses: Int): SequenceBuilder = {
    val config = Map(
      18 -> Array(
        (2, (1, 1),  64, createBasicBlock(_, _, _)),
        (2, (2, 2), 128, createBasicBlock(_, _, _)),
        (2, (2, 2), 256, createBasicBlock(_, _, _)),
        (2, (2, 2), 512, createBasicBlock(_, _, _))
      ),
      34 -> Array(
        (3, (1, 1),  64, createBasicBlock(_, _, _)),
        (4, (2, 2), 128, createBasicBlock(_, _, _)),
        (6, (2, 2), 256, createBasicBlock(_, _, _)),
        (3, (2, 2), 512, createBasicBlock(_, _, _))
      ),
      50 -> Array(
        (3, (1, 1),  64, createBottleneckBlock(_, _, _)),
        (4, (2, 2), 128, createBottleneckBlock(_, _, _)),
        (6, (2, 2), 256, createBottleneckBlock(_, _, _)),
        (3, (2, 2), 512, createBottleneckBlock(_, _, _))
      ),
      101 -> Array(
        ( 3, (1, 1),  64, createBottleneckBlock(_, _, _)),
        ( 4, (2, 2), 128, createBottleneckBlock(_, _, _)),
        (23, (2, 2), 256, createBottleneckBlock(_, _, _)),
        ( 3, (2, 2), 512, createBottleneckBlock(_, _, _))
      ),
      152 -> Array(
        ( 3, (1, 1),  64, createBottleneckBlock(_, _, _)),
        ( 8, (2, 2), 128, createBottleneckBlock(_, _, _)),
        (36, (2, 2), 256, createBottleneckBlock(_, _, _)),
        ( 3, (2, 2), 512, createBottleneckBlock(_, _, _))
      )
    )
    ...
    val c = config(depth)
    noMapsPrev = c(0)._3
    seq += createBlockGroup(c(0), isFirst = true)
    seq += createBlockGroup(c(1), isFirst = false)
    seq += createBlockGroup(c(2), isFirst = false)
    seq += createBlockGroup(c(3), isFirst = false)
    seq += BatchNormalizationBuilder()
    seq += ReLUBuilder()
