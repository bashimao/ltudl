...
final class Softmax_JVM_Baseline(override val builder:        SoftmaxBuilder,
  ...
  : Unit = {
    ...
    output.foreachSample((off, length) => {
      ...
      ArrayEx.multiply(
        out, off, 1,
        Math.min(Real(1.0 / sum), Real.maxValue),
        length
      )
    })
  }
...
object Softmax_JVM_Baseline_Description
  ...
  : Softmax_JVM_Baseline = new Softmax_JVM_Baseline(
